...
def sum_ndarray(nd):
    ...
    x=x/4.0
    y=y/4.0
    distance=(1920/2-x)**2+(1080/2-y)**2
    return distance
def draw_rect(img):
    ...
    img=cv.morphologyEx(img, cv.MORPH_CLOSE,kernel2)
    contours, hier = cv.findContours(img, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    rectboxes=[];
    for cidx,cnt in enumerate(contours):
        minAreaRect = cv.minAreaRect(cnt)
        rectCnt = np.int64(cv.boxPoints(minAreaRect))
        rectboxes.append(rectCnt)
    rectdistance=[0 for i in range(len(rectboxes))]
    print(rectdistance)
    for t in range(len(rectdistance)):
        rectdistance[t]=sum_ndarray(rectboxes[t])
    print(rectdistance)
    mini=min(rectdistance)
    print(mini)
    for t in range(len(rectdistance)):
        if rectdistance[t]==mini:
            rectfinal=rectboxes[t]
    img= cv.cvtColor(img, cv.COLOR_GRAY2BGR)
    cv.polylines(img=img, pts=[rectfinal], isClosed=True, color=(0,0,255), thickness=2)
    return img
#主程序开始 main()
...
while 1:
    ...
    lid_result=lyz.point_result(lidar_pointcloud,level5data,my_sample,'LIDAR_TOP','CAM_FRONT')
    #lid_result.get_mapped_points()
    lid_result.get_sp_points(cam_result.boxes_area[0])
    points_frame0=lid_result.show_points()
    points_frame1=lid_result.show_testpoints()
    points_frame1=draw_rect(points_frame1)
    htitch= np.hstack((points_frame1, points_frame0))
    htitch2=np.hstack((frame,filted_frame))
    cv.imshow(winName, htitch)
    cv.imshow(winName2,htitch2)
    KEY=cv.waitKey()
